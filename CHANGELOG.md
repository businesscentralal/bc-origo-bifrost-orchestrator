...
- Writes are gated with `WritePermission` and reads with `ReadPermission` (AL has no `InsertPermission` or `ModifyPermission`). Scheduler Setup ori and Job Queue Category need both before `OnOpenEmptyRec`. App Secret ori needs both, and Client Credentials ori needs read, before `Secrets.RegisterAll`. Scheduled Entry ori needs read before `IsEmpty` and write before `ModifyAll` in `SetDefaultTypeToCodeunit`. `DeleteLegacyKeys` checks `RecordRef.ReadPermission` and `RecordRef.WritePermission` before `FindSet`. Retention policies stay on the public `AddAllowedTable` API: `Retention Policy Allowed Table` is `Access = Internal` and cannot be named for a permission probe.
...
- Scheduler setup creation and `Secrets.RegisterAll` are not install-only: `Scheduler Setup ori` `OnOpenPage` already calls `OnOpenEmptyRec` and `RegisterAll`.
- Two upgrade steps are install-only. A missing grant skips them with `Session.LogMessage` (`Verbosity::Warning`, `TelemetryScope::ExtensionPublisher`): `O4NJQS-0014` for the blank Object Type to Run repair on Scheduled Entry ori, `O4NJQS-0015` for legacy isolated-storage key cleanup. These use the app prefix already used by `Session.LogMessage` in this repo (`O4NJQS-`). No upgrade tag is set, so the next upgrade retries them. `EnsureDeferredUpgradeData` runs the same steps when Scheduler Setup opens.
- Tests `Upgrade Skip Tests` (96429) and `Upgrade Recovery Tests` (96430). The skip test runs restrictive with permission set `Upg Guard Exec ori` (10035610), which can execute `Deferred Upgrade ori` (10035609) and cannot read or write the upgraded tables. That set lives in this app: a permission set in the test app cannot name an `Access = Internal` codeunit (AL0185). `EnsureDeferredUpgradeData` is a public procedure on the internal codeunit so the test app can call it through `internalsVisibleTo`. The upgrade codeunit itself is not referenceable from the test app.
