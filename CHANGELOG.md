...
- Tests `Upgrade Skip Tests` (96429) and `Upgrade Recovery Tests` (96430). The skip test runs restrictive with permission set `Upgrade Guard Exec` (96429), which can execute `Deferred Upgrade ori` (10035609) and cannot read or write the upgraded tables. The upgrade codeunit itself is not referenceable from the test app.
