...
- Two upgrade steps are install-only. A missing grant skips them with `Session.LogMessage` (`Verbosity::Warning`, `TelemetryScope::ExtensionPublisher`): `ORI-BIF-0424` for the blank Object Type to Run repair on Scheduled Entry ori, `ORI-BIF-0425` for legacy isolated-storage key cleanup. No upgrade tag is set, so the next upgrade retries them. `EnsureDeferredUpgradeData` runs the same steps when Scheduler Setup opens.
